...
def load_spikedata(path):
        ...
        for i in range(neurons.shape[0]):
                ...
                for trial in range(selected_trials.shape[1]):  # change to ...shape[2] if trial is in last dimension
                    # for every trial
                        trialID = trial

                        for position_index in range(selected_trials.shape[2]):  # change to ...shape[1] if trial is in last dimension
                                   # find position
                                   position = selected_trials[i, trial, position_index]  # exchange last two entries if trial is in last dimension
                                   if position == 0:
                                           continue
                                   else:
        ...
        # np.save(os.path.join(path_datasets_folder, "dataset_NP46_2019-12-02_18-47-02_version2.npy"), result)
